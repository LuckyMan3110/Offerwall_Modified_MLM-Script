...
<hr/>
<?php 
if(idinfo($_SESSION['uid'],"document_verified") == "1") {
    echo success($lang['success_11']);
} else {
    echo error($lang['error_22']);
}
?>
<?php if(idinfo($_SESSION['uid'],"document_verified") !== "1") { ?>
...
<div class="card">
    <div class="card-body">
        ...
        if($FormBTN == "upload") {
            ...
            $vrfdocs = $db->query("SELECT * FROM users_documents WHERE uid='$_SESSION[uid]' and document_type='$document_type'");
            ...
            if(empty($document_type)) {
                ...
                $upload_file = $secure_directory.'/'.$_SESSION[uid];
                ...
                $time = time();
                $insert = $db->query("INSERT users_documents (uid,document_type,document_path,uploaded,status,u_field_1) VALUES ('$_SESSION[uid]','$document_type','$upload_file','$time','1','$document_number')");
        ...
        <form class="user-connected-from user-signup-form" action="" method="POST" enctype="multipart/form-data">
            ...
            <button type="submit" name="upload" value="upload" class="btn btn-primary" style="padding:12px;"><?php echo $lang['btn_19']; ?></button>
        </form>
...
<div class="table-responsive">
    <table class="table table-striped">
        ...
        <tbody>
            <?php
            $GetDocuments = $db->query("SELECT * FROM users_documents WHERE uid='$_SESSION[uid]' ORDER BY id");
