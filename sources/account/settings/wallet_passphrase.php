...
$FormBTN = protect($_POST['passphrase']);
if($FormBTN == "setup") {
    $passphrase = protect($_POST['ssspassphrase']);
    if(empty($passphrase)) {
        ...
        $update = $db->query("UPDATE users SET wallet_passphrase='$passphrase' WHERE id='$_SESSION[uid]'");
...
if($FormBTN == "change") {
    ...
    if(!password_verify($cpassphrase, idinfo($_SESSION['uid'],"wallet_passphrase"))) {
...
if($FormBTN == "remove") {
    ...
    if(!password_verify($cpassphrase, idinfo($_SESSION['uid'],"wallet_passphrase"))) {
        ...
        $update = $db->query("UPDATE users SET wallet_passphrase='' WHERE id='$_SESSION[uid]'");
...
<?php
if(empty(idinfo($_SESSION['uid'],"wallet_passphrase"))) {
?>
<form class="user-connected-from user-signup-form" action="" method="POST">
<div class="form-group">
        <label><?php echo $lang['field_21']; ?></label>
        <input type="password" class="form-control" name="ssspassphrase">
    </div>
    <button type="submit" name="passphrase" value="setup" class="btn btn-primary" style="padding:12px;"><?php echo $lang['btn_20']; ?></button>
</form>
<?php 
} else {
?>
...
<div class="form-group">
    ...
    <button type="submit" name="passphrase" value="remove" class="btn btn-danger" style="padding:12px;"><?php echo $lang['btn_21']; ?></button>
    <div class="form-group">
        <label><?php echo $lang['field_21']; ?></label>
    ...
    <button type="submit" name="passphrase" value="change" class="btn btn-primary" style="padding:12px;"><?php echo $lang['btn_17']; ?></button> 
