...
$id = protect($_GET['id']);
$query = $db->query("SELECT * FROM support WHERE hash='$id' and sender='$_SESSION[uid]' or hash='$id' and recipient='$_SESSION[uid]'");
if($query->num_rows==0) { 
                                            ...
                                            if($FormBTN == "message") {
                                                ...
                                                elseif(!empty($_FILES['uploadFile']['name']) && !in_array($fileextension,$extensions)) { echo error($lang['error_1']); }
                                                    ...
                                                    if($_FILES['uploadFile']['name']) {
                                                        $path = 'uploads/disputes/'.$row['hash'].'_'.$_SESSION['uid'].'_'.time().'_attachment.'.$fileextension;
                                                        @move_uploaded_file($_FILES['uploadFile']['tmp_name'], $path); 
                                                    }
                                                    $insert = $db->query("INSERT support_messages (dispute_id,uid,comment,attachment,time,status) VALUES ('$row[id]','$_SESSION[uid]','$message','$path','$time','1')");
                                            ...
                                            <div id="submit_message">
                                                <form action="" method="POST" enctype="multipart/form-data">
                                                    ...
                                                    <div class="form-group">
                                                        ...
                                                        <button type="submit" class="btn btn-primary" name="submit" value="message"><?php echo $lang['btn_3']; ?></button>
                                                        <?php if($_SESSION['uid'] == $row['sender']) { ?>
