...
if($b == "edit") {
	...
	<div class="row">
            ...
            if($FormBTN == "profile") {
                ...
                if(isset($_POST['2fa_auth'])) { $fa_auth =1; } else { $fa_auth=0; }
                $CheckEmail = $db->query("SELECT * FROM users WHERE email='$email'");
                if(empty($first_name) or empty($last_name) or empty($email) or empty($country) or empty($city) or empty($zip_code) or empty($address) or empty($status)) {
                    ...
                    $update = $db->query("UPDATE users SET 2fa_auth='$fa_auth',first_name='$first_name',last_name='$last_name',email='$email',country='$country',city='$city',zip_code='$zip_code',address='$address',status='$status',email_verified='$email_verified',document_verified='$document_verified' WHERE id='$row[id]'");
           ...
           <div class="col-md-8">
            ...
            <div class="form-group">
			    ...
			    <div class="checkbox">
					<label>
					  <input type="checkbox" name="2fa_auth" value="yes" <?php if($row['2fa_auth'] == "1") { echo 'checked'; }?>> 2FA Auth
					</label>
			    </div>
