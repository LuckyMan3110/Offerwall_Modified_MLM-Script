    ...
    <div class="col-md-6">
        <?php
            ...
            if($FormBTN == "profile") {
                ...
                if ($type == '1') {  // ADD
                    UpdateUserWallet($id,$amount,$currency,1); //Wallet Add
                    $create_transaction = $db->query("INSERT transactions (txid,type,sender,description,amount,currency,fee,status,created) VALUES ('$txid','28','$_SESSION[uid]','$description','$amount','$currency','0.00','1','$time')");
                    $insert_activity = $db->query("INSERT activity (txid,type,uid,amount,currency,status,created) VALUES ('$txid','28','$_SESSION[uid]','$amount','$currency','1','$time')");
                    echo success("Fund has been Added.");  
                } elseif ($type == '2') { // Subtract
                    UpdateUserWallet($id,$amount,$currency,2); //Wallet Deduction
