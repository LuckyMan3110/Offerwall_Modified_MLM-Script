...
function Load_Gateway_Fields(id) {
	var url = $("#url").val();
	var data_url = url + "requests/Load_Gateway_Fields.php?id="+id;
	$.ajax({
		type: "GET",
		url: data_url,
		dataType: "html",
		success: function (data) {
			$("#gateway_fields").html(data);
		}
	});
}

function GetWalletCurrency(id) {
    var url = $("#url").val();
	var data_url = url + "requests/GetWalletCurrency.php?id="+id;
	$.ajax({
		type: "GET",
		url: data_url,
		dataType: "html",
		success: function (data) {
            ...
            if(c_currency !== d_currency) {
                currencyConverter(fee,d_currency,c_currency);           
            } else {
                $("#c_fee").val(fee);
...
function CurrencyConverter(amount,from,to) {
    var url = $("#url").val();
	var data_url = url + "requests/CurrencyConverter.php?amount="+amount+"&from="+from+"&to="+to;
	$.ajax({
		type: "GET",
		url: data_url,
		dataType: "html",
...
$(function() {
    ...
    $.LoadRates = function() {
        ...
        var data_url = url + "requests/Convert.php?from="+from_currency+"&to="+to_currency;
        $.ajax({
            type: "GET",
            url: data_url,
            dataType: "json",
