...
function GetRates($from_currency,$to_currency) {
	global $db, $settings;
	$data = array();
	if(empty($from_currency) or empty($to_currency)) {
		...
		$fee = 0;
		$rate_from = 1;
		$calculate = currencyConvertor($rate_from,$from_currency,$to_currency);
		$calculate1 = ($calculate * $fee) / 100;
		$calculate2 = $calculate - $calculate1;
		if($calculate2 < 1) { 
			$calculate = currencyConvertor($rate_from,$to_currency,$from_currency);
			$calculate1 = ($calculate * $fee) / 100;
			$calculate2 = $calculate - $calculate1;
...
function UpdateMembership($txid,$plan,$uid,$amount,$currency,$status,$time,$date,$date_complete) {
	global $db;
	$CheckMembership = $db->query("SELECT * FROM membership_log WHERE uid='$uid' and currency='$currency'");
	if($CheckMembership->num_rows>0) {
		$row = $CheckMembership->fetch_assoc();
		$update = $db->query("UPDATE membership_log SET txid='$txid',plan='$plan',amount='$amount',currency='$currency',status='$status',created='$time',date='$date',end_date='$date_complete' WHERE uid='$uid'");
	} else {
	    $insert_membership_log = $db->query("INSERT membership_log (txid,plan,uid,amount,currency,status,created,date,end_date) 
		VALUES ('$txid','$plan','$uid','$amount','$currency','$status','$time','$date','$date_complete')");
	}
}

function UpdateUserWallet($uid,$amount,$currency,$type) {
	global $db;
...
function DecodeUserActivity($activity_id) {
	global $db, $lang;
	...
	if($ActivityQuery->num_rows>0) {
		...
		if($type == "1") {
			...
			$activity_8 = "Membership Purchased";
			return $activity_8;
		} elseif($type == "300") {
			$activity_8 = "Referral Commission Earned";
			return $activity_8;
		} elseif($type == "30") {
			...
			// Stripe api deposit (+)	
			$activity_8 = "Payment for Claimed Reward";
			return $activity_8;
		} elseif($type == "31") {
			// Offerwall Add	
			$activity_8 = "Earning from Offerwalls";
			return $activity_8;
		} elseif($type == "32") {
			// Offerwall Add	
			$activity_8 = "Earning from Offerwalls";
...
function DecodeTXStatus($status) {
	global $db, $lang;
	if($status == "1") {
...
function UpdateAdminWallet($amount,$currency) {
	global $db;
	$time = time();
