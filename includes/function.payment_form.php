...
function PF_Stripe($id,$gateway) {
	...
	$_SESSION['stripe_productName'] = $productName;
	$_SESSION['stripe_productNumber'] = $productNumber;
	$_SESSION['stripe_productPrice'] = $productPrice;
	$_SESSION['stripe_gateway'] = $gateway;
	$_SESSION['stripe_currency'] = $currency;
	$_SESSION['stripe_stripeAmount'] = $stripeAmount;
	$return .= '<script type="text/javascript" src="'.$settings['url'].'assets/js/jquery-1.12.4.min.js"></script>';
...
function PF_m_1($id,$gateway)
{
	...
	$process_type = gatewayinfo($gateway,"process_type");
			...
			$return .= '
				...
				$return .= '<br><center style="color:#fff;">Deposit will be processed up to '.$processed_time.' after payment.</center><br/>';
